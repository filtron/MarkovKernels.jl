function likelihood_test(T, n, m, affine_types, cov_types)
    ...
    normal_loglikes = map( x-> LogLike(x...), zip(normal_kernels, fill(y, length(normal_kernels))) )
    dirac_loglikes = map(  x-> LogLike(x...), zip(dirac_kernels, fill(y, length(dirac_kernels))) )
