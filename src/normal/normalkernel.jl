...
condition(K::NormalKernel{T,U,V},x) where {T,U<:AbstractConditionalMean,V<:AbstractMatrix} = Normal( mean(K)(x), cov(K) )
condition(K::DiracKernel,x) = Dirac(mean(K)(x))

compose(K2::NormalKernel{T,U,V},K1::NormalKernel{T,U,V}) where {T,U<:AbstractAffineMap,V<:AbstractMatrix} = NormalKernel( compose(mean(K2), mean(K1)), stein(cov(K1), mean(K2), cov(K2)) )
compose(K2::DiracKernel{T,U},K1::DiracKernel{T,U}) where {T,U<:AbstractAffineMap} = DiracKernel( compose(mean(K2),mean(K1)) )
compose(K2::DiracKernel{T,U},K1::NormalKernel{T,U,V}) where {T,U<:AbstractAffineMap,V<:AbstractMatrix} = NormalKernel( compose(mean(K2), mean(K1)), stein(cov(K1), mean(K2), cov(K2)) )
compose(K2::NormalKernel{T,U,V},K1::DiracKernel{T,U}) where {T,U<:AbstractAffineMap,V<:AbstractMatrix} = NormalKernel( compose(mean(K2), mean(K1)), cov(K2) )

*(K2::AbstractNormalKernel,K1::AbstractNormalKernel)  =  compose(K2,K1)

marginalise(N::Normal{T,U,V},K::NormalKernel{T,S,V}) where {T,U,S<:AbstractAffineMap,V<:AbstractMatrix} = Normal( mean(K)(mean(N)), stein(cov(N), mean(K), cov(K)) )
marginalise(N::Normal{T,U,V},K::DiracKernel{T,S}) where {T,U,S<:AbstractAffineMap,V<:AbstractMatrix} = Normal( mean(K)(mean(N)), stein(cov(N), mean(K), cov(K)) ) # not the smartest way ...
