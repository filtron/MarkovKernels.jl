zero(a::LinearMap) = LinearMap(zero(slope(a)))
zero(a::AffineMap) = AffineMap(zero(slope(a)), zero(intercept(a)))
zero(a::AffineCorrector) = AffineCorrector(zero(a.A), zero(a.b), zero(a.c))
...
# some fixes required for SISO/SIMO/MISO
similar(a::LinearMap, ::Type{T}, dims::NTuple{2, IT}) where {T,IT} = LinearMap(similar(a.A, T, dims))
similar(a::AffineMap, ::Type{T}, dims::NTuple{2, IT}) where {T,IT} = AffineMap(similar(a.A, T, dims), similar(a.b, T, first(dims)))
similar(a::AffineCorrector, ::Type{T}, dims::NTuple{2, IT}) where {T,IT} = AffineCorrector(similar(a.A, T, dims), similar(a.b, T, first(dims)), similar(a.c, T, last(dims)))
...
similar(a::AT) where {AT<:AbstractAffineMap} = AT(Tuple(similar(getfield(a, f)) for f in fieldnames(AT))...)
